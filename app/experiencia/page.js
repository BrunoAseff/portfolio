...
const certifications = [
  {
    ...
    date: "Emitido em 29 de Junho de 2025",
  ...
  },
   {
    ...
    credentialUrl: "https://www.linkedin.com/in/bruno-aseff/details/certifications/1728333429207/single-media-viewer/?type=DOCUMENT&profileId=ACoAADSgZsgBmJ1zfWrk5RJbJD7u5DnJPpqQ1PE", 
  ...
  },
  {
    ...
    issuer: "Curso em Vídeo",
    date: "Emitido em 03/09/2024",
    credentialUrl: "https://www.linkedin.com/in/bruno-aseff/details/certifications/1728333326902/single-media-viewer/?type=DOCUMENT&profileId=ACoAADSgZsgBmJ1zfWrk5RJbJD7u5DnJPpqQ1PE", 
...
const GlassCard = ({ children, className = '' }) => (
  <div className={`bg-white/10 backdrop-blur-lg border border-white/20 shadow-lg rounded-2xl p-6 transition-all duration-300 hover:bg-white/20 hover:shadow-xl ${className}`}>
    {children}
  </div>
);
  ...
  <div className="md:bg-white/10 md:backdrop-blur-lg md:border md:border-white/20 md:shadow-lg rounded-2xl p-2 mb-6">
    <div className="flex justify-center items-center gap-2">
      ...
      <button
        ...
        className={`w-full text-center px-6 py-3 rounded-xl font-semibold transition-all duration-300 flex items-center justify-center gap-2 ${activeView === 'certifications' ? 'bg-white/30 text-white shadow-md' : 'text-white/70 hover:bg-white/10'}`}
      >
    ...
    <GlassCard className="mb-6 animate-fade-in">
        <div className="flex flex-col sm:flex-row items-start gap-6">
            ...
            <div className="flex-grow">
                ...
                <p className="text-md font-semibold text-white/80">Emitido por: {certification.issuer}</p>
                <p className="text-sm text-white/60 mb-4">{certification.date}</p>
                <div className="flex flex-wrap gap-2 mb-4">
                    {certification.skills.map((skill, index) => (
                        <span key={index} className="bg-white/10 text-white/80 text-xs font-medium px-3 py-1 rounded-full border border-white/20">
                            {skill}
                        </span>
                    ))}
                </div>
